...
router.post('/', async (req, res, next) => { try { const r = await svc.createSchedule(req.body, req.userId); res.json({ id: r.id, message: 'Schedule created successfully' }); } catch(e){next(e);} });
