...
const syncTenantAccess = async (tenant_id) => {
  try {
    ...
    for (const device of devices) {
      ...
      let holidayId = 0;
      if (access && access.access_group_id && isApproved) {
        const group = await db('access_groups').where({ id: access.access_group_id, user_id: tenant.user_id }).first();
      ...
      if (isApproved) {
        ...
        // Define an explicit restricted timezone (50) and group (50) for blocked users
        commands.push({ device_sn: device.sn, command: `DATA UPDATE timezone TimezoneId=50\tSUN=00:00-00:00\tMON=00:00-00:00\tTUE=00:00-00:00\tWED=00:00-00:00\tTHU=00:00-00:00\tFRI=00:00-00:00\tSAT=00:00-00:00`, user_id: tenant.user_id });
        commands.push({ device_sn: device.sn, command: `DATA UPDATE accgroup id=50\ttimezone1=50\ttimezone2=0\ttimezone3=0\tholiday=0\tverifystyle=0`, user_id: tenant.user_id });
      }

      // Grant/Update: DATA UPDATE USERINFO
      const cleanName = tenant.name.replace(/[^\w]/g, '');
      const tzMask = isApproved ? generateTZMask(tzIds[0]) : generateTZMask(50);

      let extraParams = [];
      if (tenant.access_expiry_date && isApproved) {
      ...
      commands.push({
        device_sn: device.sn,
        command: `DATA UPDATE USERINFO PIN=${pin}\tName=${cleanName}\tPri=0\tPass=\tCard=\tGrp=${isApproved ? grpId : 50}\tTZ=${tzMask}\tPIN2=${pin}${extraString}`,
        user_id: tenant.user_id
      });
